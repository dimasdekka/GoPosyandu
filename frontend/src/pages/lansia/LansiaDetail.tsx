...
export default function LansiaDetail() {
  ...
  const [resikoPTM, setResikoPTM] = useState<
  ...
  const onSubmitPemeriksaan = async () => {
    ...
    try {
      ...
      if (parseInt(tensiSistolik) > 0)
        payload.tensiSistolik = parseInt(tensiSistolik);
      if (parseInt(tensiDiastolik) > 0)
        payload.tensiDiastolik = parseInt(tensiDiastolik);
  ...
  if (!profile) {
    return (
      <div className="space-y-6">
        <Button variant="outline" onClick={() => navigate("/lansia")}>
          <ArrowLeft className="w-4 h-4 mr-2" /> Kembali
        </Button>
        <div className="flex items-center justify-center min-h-[50vh]">
          <p className="text-destructive">Data Lansia tidak ditemukan.</p>
        </div>
      </div>
    );
  }
  ...
  const latestP =
    profile.pemeriksaan?.length > 0 ? profile.pemeriksaan[0] : null;
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <Badge
                variant="outline"
                className={`border ${getResikoBadge(currentResiko)}`}
              >
                {currentResiko}
              </Badge>
            </div>
        ...
        >
          ...
          <DialogContent className="sm:max-w-[560px] max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Catat hasil screening kesehatan lansia.{" "}
              ...
              <div className="space-y-4 py-2">
                ...
                <div className="space-y-1">
                  <Label className="text-sm">
                    Risiko PTM <span className="text-destructive">*</span>
                  </Label>
                  <Select
                    value={resikoPTM}
                    onValueChange={(v) => setResikoPTM(v as any)}
                  >
                    <SelectTrigger>
                    ...
                    <SelectContent>
                      <SelectItem value="Rendah">
                        ✅ Rendah — Tidak ada faktor risiko
                      </SelectItem>
                      <SelectItem value="Sedang">
                        ⚠️ Sedang — 1-2 faktor risiko
                      </SelectItem>
                      <SelectItem value="Tinggi">
                        🔴 Tinggi — &gt;2 faktor risiko / sudah PTM
                      </SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              ...
              <div className="pt-4 mt-2 border-t border-dashed border-border/60 space-y-4">
                ...
                <div className="space-y-3.5">
                  <div className="flex justify-between items-end border-b border-dashed border-border/30 pb-1.5">
                    ...
                    <span
                      className={`font-bold text-sm ${latestP.tensiSistolik >= 140 ? "text-destructive" : ""}`}
                    >
                      {latestP.tensiSistolik || "-"}/
                      {latestP.tensiDiastolik || "-"}{" "}
                  ...
                  <div className="flex justify-between items-end border-b border-dashed border-border/30 pb-1.5">
                    ...
                    <span
                      className={`font-bold text-sm ${latestP.gulaDarah >= 140 ? "text-destructive" : ""}`}
                    >
                      {latestP.gulaDarah || "-"}{" "}
                      <span className="text-[10px] font-normal text-muted-foreground ml-1">
                        mg/dL
                  ...
                  <div className="flex justify-between items-end border-b border-dashed border-border/30 pb-1.5">
                    ...
                    <span className="font-bold text-sm text-primary">
                      AU:{" "}
                      {latestP.asamUrat != null
                        ? Number(latestP.asamUrat).toFixed(1)
                        : "-"}{" "}
                      <span className="mx-1 text-muted-foreground">•</span> Kol:{" "}
                      {latestP.kolesterol || "-"}
                    </span>
                  </div>
                  <div className="flex justify-between items-end">
                    ...
                    <span className="font-bold text-sm">
                      BB:{" "}
                      {latestP.bb != null ? Number(latestP.bb).toFixed(1) : "-"}{" "}
                      kg <span className="mx-1 text-muted-foreground">|</span>{" "}
                      TB:{" "}
                      {latestP.tb != null ? Number(latestP.tb).toFixed(1) : "-"}{" "}
                      cm
                    </span>
                  </div>
                </div>
        ...
        <Card className="md:col-span-2 border-border/60 shadow-sm">
          <CardHeader>
            <CardTitle className="flex items-center">
              Riwayat Screening PTM
            </CardTitle>
            <CardDescription>
              Pengecekan Tensi, Gula Darah, Asam Urat, dan Kolesterol.
            </CardDescription>
          </CardHeader>
              ...
              <TableBody>
                {profile.pemeriksaan?.length > 0 ? (
                  profile.pemeriksaan.map((r: any) => (
                    <TableRow key={r.id} className="hover:bg-muted/30">
                      <TableCell className="font-medium whitespace-nowrap">
                        {new Date(
                          r.tglPeriksa || r.createdAt,
                        ).toLocaleDateString("id-ID")}
                      </TableCell>
                      <TableCell className="text-sm">
                        <div>
                          {r.bb != null ? `${Number(r.bb).toFixed(1)} kg` : "-"}
                        </div>
                        <div className="text-muted-foreground">
                          {r.tb != null ? `${Number(r.tb).toFixed(1)} cm` : ""}
                        </div>
                      </TableCell>
                      <TableCell>
                        {r.tensiSistolik
                      ...
                      <TableCell>
                        <Badge
                          variant="outline"
                          className={`border ${getResikoBadge(r.resikoPTM)}`}
                        >
                          {r.resikoPTM}
                        </Badge>
                      </TableCell>
                  ...
                  <TableRow>
                    ...
                    >
                      Belum ada riwayat screening. Klik "Screening PTM Baru"
                      untuk memulai.
                    </TableCell>
                  </TableRow>
                )}
