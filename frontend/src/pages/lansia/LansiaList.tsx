...
export default function LansiaList() {
  ...
  const fetchLansia = async () => {
    try {
      ...
      const mapped = raw.map((item: any) => {
        ...
        const tensi = latestP
          ? `${latestP.tensiSistolik ?? "-"}/${latestP.tensiDiastolik ?? "-"}`
          : "-";
  ...
  const addForm = useForm<FormValues>({
    ...
    defaultValues: { nama: "", umur: "", jk: "L", alamat: "" },
  });
  ...
  const onEdit = async (values: FormValues) => {
    ...
    try {
      setSubmitting(true);
      await axios.patch(`/api/v1/lansia/${editTarget.id}`, {
        ...values,
        umur: parseInt(values.umur),
      });
      setEditTarget(null);
  ...
  const filteredData = data.filter((item) => {
    ...
    const matchStatus =
      filterStatus === "Semua" || item.resikoPTM === filterStatus;
    ...
    <>
      <FormField
        ...
        render={({ field }) => (
          <FormItem>
            ...
            <FormControl>
              <Input placeholder="Nama lansia/dewasa" {...field} />
            </FormControl>
            <FormMessage />
          </FormItem>
        )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input type="number" placeholder="Contoh: 65" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
      ...
      <div className="pt-2 border-t border-border/50 mt-4">
        <p className="text-xs font-semibold text-primary uppercase tracking-wider mb-3">
          Hasil Pemeriksaan Hari Ini (Opsional)
        </p>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">
            Pencatatan PTM (Penyakit Tidak Menular) dan Screening Lansia.
          </p>
        </div>
        <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
          <DialogTrigger asChild>
            <Button className="w-full sm:w-auto shadow-sm transition-all hover:scale-[1.02] active:scale-95">
          ...
          <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              ...
              <DialogDescription>
                Masukkan biodata lengkap peserta posyandu lansia.
              </DialogDescription>
            </DialogHeader>
            <Form {...addForm}>
              <form
      ...
      >
        <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Perbarui informasi identitas peserta.
            </DialogDescription>
          </DialogHeader>
          <Form {...editForm}>
            <form
      ...
      <Card className="border-border/60 shadow-sm">
        <CardContent className="p-4 flex flex-col sm:flex-row gap-4 items-center justify-between">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            <select
              className="flex h-10 w-full sm:w-[220px] rounded-md border border-input bg-transparent px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring"
              value={filterStatus}
                ...
                filteredData.map((item, idx) => (
                  <TableRow key={item.id} className="hover:bg-muted/30">
                    ...
                    <TableCell>
                      ...
                      <div className="text-[10px] text-muted-foreground visible sm:hidden">
                        {item.umur} Thn
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-sm">
                      ...
                      <div className="text-muted-foreground text-[11px] flex flex-wrap gap-x-2">
                        <span>
                          Gula: {item.gula != null ? `${item.gula} mg/dL` : "-"}
                        </span>
                        <span className="opacity-30">|</span>
                        <span>
                          BB:{" "}
                          {item.bb != null ? Number(item.bb).toFixed(1) : "-"}
                        </span>
                        <span className="opacity-30">|</span>
                        <span>
                          TB:{" "}
                          {item.tb != null ? Number(item.tb).toFixed(1) : "-"}
                        </span>
                      </div>
                    </TableCell>
                    <TableCell>
                      <Badge
                        variant="outline"
                        className={`border ${getStatusBadge(item.resikoPTM)}`}
                      >
                        {item.resikoPTM}
                      </Badge>
                    </TableCell>
