...
const pemeriksaanSchema = z.object({
  ...
  lingkarPerut: z.number().positive().optional(),
  kadarHb: z.number().positive().optional(),
  statusGizi: z.enum(['Normal', 'Kurang', 'Lebih', 'Anemia']),
...
export class RemajaController {
  ...
  async update(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await service.updateRemaja(id, validatedData);
      res.json({ success: true, data: result });
    } catch (error) {
      if (error instanceof z.ZodError) return res.status(400).json({ success: false, errors: error.issues });
      next(error);
